...
//
// Layout:
//   LEFT  panel  — Breadcrumb + Systemic tree (drill-down in-place)
//   RIGHT panel  — Therapeutic class cards grid
//
// Modals (dialogs stacked on top):
//   Therapeutic tap  → _GenericsModal   (grid of generic cards)
//   Generic tap      → _BrandsModal     (searchable brand list)
//   Brand tap        → _BrandDetailModal (full BrandDetailView)
// ─────────────────────────────────────────────────────────────────────────────
...
class _DrugClassScreenState extends State<DrugClassScreen> {
  ...
  final SystemicClassCtrl _systemicCtrl = Get.put(SystemicClassCtrl());
  final TherapeuticClassCtrl _therapeuticCtrl = Get.put(TherapeuticClassCtrl());
  final TherapeuticGenIndCtrl _tgiCtrl =
  Get.put(TherapeuticGenIndCtrl());
  final GenericCtrl _genericCtrl = Get.put(GenericCtrl());
  final DrugBrandCtrl _brandCtrl = Get.put(DrugBrandCtrl());

  /// Breadcrumb stack — each entry is a SystemicClassModel we drilled into.
  /// Empty = root level (parent_id == 0).
  final List<SystemicClassModel> _breadcrumb = [];

  /// The leaf systemic that was tapped (has no children → shows therapeutics).
  ...
  int get _currentParentId =>
      _breadcrumb.isEmpty ? 0 : _breadcrumb.last.id;

  List<SystemicClassModel> _childrenOf(int parentId) =>
      _systemicCtrl.systemicClassList
          .where((s) => (s.parent_id ?? 0) == parentId)
          .toList();

  bool _hasChildren(SystemicClassModel s) =>
      _systemicCtrl.systemicClassList
          .any((c) => (c.parent_id ?? 0) == s.id);

  List<TherapeuticClassModel> _therapeuticsFor(int systemicId) =>
      _therapeuticCtrl.therapeuticClassList
          .where((t) => t.systemic_class_id == systemicId)
          .toList();
  ...
  List<GenericDetailsModel> _genericsFor(int therapeuticId) {
    final ids = _tgiCtrl.therapeuticGenIndList
        ...
        .map((x) => x.generic_id)
        .toSet();
    return _genericCtrl.genericList
        .where((g) => ids.contains(g.generic_id))
        .toList();
  }
  ...
  void _openGenericsModal(TherapeuticClassModel t, ThemeDefinition theme, double fss) {
    showDialog(
      ...
      builder: (_) => _GenericsModal(
        ...
        companyCtrl: Get.put(CompanyCtrl()),
  ...
  Widget build(BuildContext context) {
    return Obx(() {
      ...
      final visibleChildren = _childrenOf(_currentParentId).where((s) =>
      _systemicQuery.isEmpty ||
          s.name.toLowerCase().contains(_systemicQuery)).toList();

      final therapeutics = _selectedSystemic != null
          ? _therapeuticsFor(_selectedSystemic!.id)
          : <TherapeuticClassModel>[];

      return Row(
        children: [
          // ── LEFT: Systemic tree panel ──────────────────────────────────────
          Container(
            width: 280 * fss,
              ...
              children: [
                ...
                Container(
                  padding: EdgeInsets.fromLTRB(
                      16 * fss, 14 * fss, 16 * fss, 14 * fss),
                  decoration: BoxDecoration(
                    border: Border(bottom: BorderSide(color: theme.divider)),
                  ),
                  child: Row(children: [
                    Icon(Icons.category_outlined,
                        size: 15 * fss, color: theme.accent),
                ...
                Padding(
                  ...
                  child: SearchInput(
                    ...
                    hint: 'Search classes...',
                    onChanged: (q) =>
                        setState(() => _systemicQuery = q.toLowerCase()),
                    fontSizeScale: fss,
                ...
                Expanded(
                  child: visibleChildren.isEmpty
                    ...
                    child: Text(
                      'No classes found',
                      style: TextStyle(
                          fontSize: 12 * fss, color: theme.textMuted),
                    ),
                  )
                      : ListView.builder(
          ...
          // ── RIGHT: Therapeutic cards grid ──────────────────────────────────
          Expanded(
            child: _selectedSystemic == null
                ? _EmptyState(
              icon: Icons.account_tree_outlined,
              title: _breadcrumb.isEmpty
                  ? 'Select a Drug Class'
                  : 'Select a Sub-Class',
              ...
              onTherapeuticTap: (t) =>
                  _openGenericsModal(t, theme, fss),
              theme: theme,
              fss: fss,
            ),
          ),
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      padding: EdgeInsets.symmetric(
          horizontal: 12 * fss, vertical: 8 * fss),
      decoration: BoxDecoration(
        color: theme.bg,
        border:
        Border(bottom: BorderSide(color: theme.divider.withOpacity(0.6))),
      ),
          ...
          children: [
            ...
            ...breadcrumb.asMap().entries.map((e) {
              ...
              return Row(children: [
                Padding(
                  padding:
                  EdgeInsets.symmetric(horizontal: 4 * fss),
                  child: Icon(Icons.chevron_right_rounded,
                      size: 13 * fss, color: theme.textMuted),
                ),
                _CrumbChip(
                  label: e.value.name,
                  isLast: isLast,
                  onTap: isLast ? null : () => onJump(e.key),
                  theme: theme,
                  fss: fss,
                ),
              ]);
  ...
  Widget build(BuildContext context) {
    return MouseRegion(
      ...
      child: GestureDetector(
        ...
        child: Container(
          padding: EdgeInsets.symmetric(
              horizontal: 8 * fss, vertical: 3 * fss),
          decoration: BoxDecoration(
            color: isLast
          ...
          child: Row(
            ...
            children: [
              if (icon != null) ...[
                Icon(icon,
                    size: 11 * fss,
                    color: isLast ? theme.accent : theme.textSecondary),
        ...
        child: AnimatedContainer(
          ...
          decoration: BoxDecoration(
            color: widget.isSelected
                ...
                : _hover
                ? widget.theme.surfaceHighlight
                : Colors.transparent,
            border: Border(
              bottom: BorderSide(
                  color: widget.theme.divider.withOpacity(0.4)),
              left: BorderSide(
                color: widget.isSelected
                    ? widget.theme.accent
                    : Colors.transparent,
          ...
          padding: EdgeInsets.symmetric(
              horizontal: 16 * widget.fss, vertical: 11 * widget.fss),
          child: Row(
            children: [
              Container(
                width: 30 * widget.fss,
                ...
                decoration: BoxDecoration(
                  color: widget.isSelected
                      ...
                      : widget.theme.surfaceElevated,
                  borderRadius:
                  BorderRadius.circular(7 * widget.fss),
                ),
                child: Icon(
                  widget.hasChildren
      ...
      children: [
        ...
        Container(
          ...
          child: Row(children: [
            Container(
              padding: EdgeInsets.symmetric(
                  horizontal: 10 * fss, vertical: 5 * fss),
              decoration: BoxDecoration(
              ...
              child: Row(mainAxisSize: MainAxisSize.min, children: [
                Icon(Icons.local_hospital_outlined,
                    size: 13 * fss, color: theme.accent),
            ...
            Text(
              '${therapeutics.length} therapeutic classes',
              style: TextStyle(fontSize: 12 * fss, color: theme.textMuted),
            ),
          ]),
        ),
        ...
        Expanded(
          child: therapeutics.isEmpty
            ...
            title: 'No Therapeutic Classes',
            subtitle:
            'No therapeutic classes linked to "${systemic.name}"',
            theme: theme,
            fss: fss,
          )
              : GridView.builder(
            ...
            gridDelegate: SliverGridDelegateWithMaxCrossAxisExtent(
              maxCrossAxisExtent: 220 * fss,
              mainAxisSpacing: 12 * fss,
              crossAxisSpacing: 12 * fss,
              childAspectRatio: 1.5,
            ),
            itemCount: therapeutics.length,
            itemBuilder: (_, i) => _TherapeuticCard(
              therapeutic: therapeutics[i],
              color: _kCardAccents[i % _kCardAccents.length],
              onTap: () => onTherapeuticTap(therapeutics[i]),
              theme: theme,
              fss: fss,
            ),
          ),
        ...
        child: AnimatedContainer(
          ...
          child: Column(
            ...
            children: [
              Container(
                width: 32 * widget.fss,
                height: 32 * widget.fss,
                decoration: BoxDecoration(
                  color: widget.color.withOpacity(0.15),
                  borderRadius: BorderRadius.circular(8 * widget.fss),
                ),
                child: Icon(Icons.medication_outlined,
                    size: 16 * widget.fss, color: widget.color),
              ),
              Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
...
class _GenericsModalState extends State<_GenericsModal> {
  ...
  void _openBrandsModal(GenericDetailsModel g) {
    showDialog(
      context: context,
      builder: (_) => _BrandsModal(
        generic: g,
        brands: widget.brandCtrl.getBrandsByGeneric(g.generic_id),
        companyCtrl: widget.companyCtrl,
        themeCtrl: widget.themeCtrl,
        theme: widget.theme,
        fss: widget.fss,
      ),
    );
  ...
  Widget build(BuildContext context) {
    ...
    final displayed = _query.isEmpty
        ...
        : widget.generics
        .where((g) =>
        g.generic_name.toLowerCase().contains(_query))
        .toList();

    return Dialog(
      backgroundColor: Colors.transparent,
      insetPadding: EdgeInsets.symmetric(
          horizontal: 80 * fss, vertical: 60 * fss),
      child: Container(
        constraints: BoxConstraints(maxWidth: 900 * fss),
        decoration: _modalDecoration(theme, fss),
        child: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            _ModalHeader(
              title: widget.therapeutic.name,
              subtitle: '${displayed.length} generics',
            ...
            Flexible(
              child: displayed.isEmpty
                ...
                fss: fss,
                  ...
                  : GridView.builder(
                padding: EdgeInsets.fromLTRB(
                    20 * fss, 4 * fss, 20 * fss, 20 * fss),
                shrinkWrap: true,
                gridDelegate:
                SliverGridDelegateWithMaxCrossAxisExtent(
                  maxCrossAxisExtent: 190 * fss,
                  mainAxisSpacing: 10 * fss,
                  crossAxisSpacing: 10 * fss,
                  childAspectRatio: 1.55,
                ),
                itemCount: displayed.length,
                itemBuilder: (_, i) => _GenericCard(
                  generic: displayed[i],
                  onTap: () => _openBrandsModal(displayed[i]),
                  theme: theme,
                  fss: fss,
                ),
              ),
        ...
        child: AnimatedContainer(
          ...
          decoration: BoxDecoration(
            color: _hover
                ...
                : widget.theme.bg,
            borderRadius: BorderRadius.circular(10 * widget.fss),
            border: Border.all(
              color: _hover
              ...
              Column(
                ...
                children: [
                  ...
                  if (widget.generic.pregnancy_category_id != null) ...[
                    ...
                    Container(
                      padding: EdgeInsets.symmetric(
                          horizontal: 5 * widget.fss,
                          vertical: 1 * widget.fss),
                      decoration: BoxDecoration(
                        color: AppTheme.accentAmber.withOpacity(0.15),
                        borderRadius:
                        BorderRadius.circular(3 * widget.fss),
                      ),
                      child: Text(
                        'Cat ${widget.generic.pregnancy_category_id}',
                        style: TextStyle(
                          fontSize: 9 * widget.fss,
...
class _BrandsModalState extends State<_BrandsModal> {
  ...
  List<DrugBrandModel> get _displayed {
    ...
    if (_query.isNotEmpty) {
      list = list
          .where((b) =>
      b.brand_name.toLowerCase().contains(_query) ||
          (b.strength?.toLowerCase().contains(_query) ?? false) ||
          (b.form?.toLowerCase().contains(_query) ?? false))
          .toList();
    }
  ...
  List<int> get _companyIds =>
      widget.brands.map((b) => b.company_id).toSet().toList()..sort();

  void _openBrandDetail(DrugBrandModel brand) {
    showDialog(
      context: context,
      builder: (_) => _BrandDetailModal(
        brand: brand,
        theme: widget.theme,
        fss: widget.fss,
      ),
  ...
  Widget build(BuildContext context) {
    ...
    return Dialog(
      backgroundColor: Colors.transparent,
      insetPadding:
      EdgeInsets.symmetric(horizontal: 60 * fss, vertical: 40 * fss),
      child: Container(
        constraints:
        BoxConstraints(maxWidth: 1000 * fss, maxHeight: 680 * fss),
        decoration: _modalDecoration(theme, fss),
        child: Column(
          children: [
            _ModalHeader(
              title: widget.generic.generic_name,
              subtitle: '${displayed.length} of ${widget.brands.length} brands',
              icon: Icons.local_pharmacy_outlined,
              iconColor: AppTheme.accentPurple,
              onClose: () => Navigator.pop(context),
              theme: theme,
              fss: fss,
              trailing: widget.generic.pregnancy_category_id != null
                  ? _PregnancyBadge(
                  catId: widget.generic.pregnancy_category_id!,
                  fss: fss)
                  : null,
            ),
            ...
            Padding(
              ...
              child: Row(children: [
                Expanded(
                  ...
                  child: SearchInput(
                    controller: _search,
                    hint: 'Search by name, strength or form...',
                    onChanged: (q) =>
                        setState(() => _query = q.toLowerCase()),
                    fontSizeScale: fss,
            ...
            Expanded(
              child: displayed.isEmpty
                ...
                fss: fss,
                  ...
                  : ListView.builder(
                padding: EdgeInsets.fromLTRB(
                    16 * fss, 4 * fss, 16 * fss, 16 * fss),
                itemCount: displayed.length,
                itemBuilder: (_, i) {
                  final b = displayed[i];
                  final company =
                  widget.companyCtrl.getCompanyById(b.company_id);
                  return _BrandRow(
                    brand: b,
                    companyName: company?.company_name,
                    onTap: () => _openBrandDetail(b),
                    theme: theme,
                    fss: fss,
                    showPrice:
                    widget.themeCtrl.showPriceInList.value,
                  );
                },
              ),
            ),
      ...
      child: GestureDetector(
        ...
        child: AnimatedContainer(
          ...
          decoration: BoxDecoration(
            ...
            border: Border.all(
              color: _hover
                  ? AppTheme.accentPurple.withOpacity(0.28)
                  : Colors.transparent,
            ),
          ),
          padding: EdgeInsets.symmetric(
              horizontal: 14 * widget.fss, vertical: 10 * widget.fss),
          child: Row(
            children: [
              Container(
                width: 36 * widget.fss,
                height: 36 * widget.fss,
                decoration: BoxDecoration(
                  color: AppTheme.accentPurple.withOpacity(0.1),
                  borderRadius:
                  BorderRadius.circular(8 * widget.fss),
                ),
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Wrap(spacing: 5, runSpacing: 3, children: [
                      if (b.strength != null)
                        _Chip(b.strength!, widget.theme.accent, widget.fss),
                      if (b.form != null)
                        _Chip(b.form!, AppTheme.accentGreen, widget.fss),
                    ]),
              ...
              Column(
                ...
                children: [
                  if (b.price != null && widget.showPrice)
                    Container(
                      padding: EdgeInsets.symmetric(
                          horizontal: 8 * widget.fss,
                          vertical: 4 * widget.fss),
                      decoration: BoxDecoration(
                        color: AppTheme.accentGreen.withOpacity(0.1),
                        borderRadius:
                        BorderRadius.circular(6 * widget.fss),
                      ),
                      child: Text(
                        '৳${b.price}',
                        style: TextStyle(
...
// ─────────────────────────────────────────────────────────────────────────────
// MODAL 3 — Brand Detail
// ─────────────────────────────────────────────────────────────────────────────

class _BrandDetailModal extends StatelessWidget {
  final DrugBrandModel brand;
  final ThemeDefinition theme;
  final double fss;

  const _BrandDetailModal({
    required this.brand,
    required this.theme,
    required this.fss,
  });
  ...
  Widget build(BuildContext context) {
    return Dialog(
      ...
      child: Container(
        constraints: BoxConstraints(maxWidth: 1100 * fss),
        decoration: _modalDecoration(theme, fss),
        child: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            ...
            Container(
              padding: EdgeInsets.symmetric(
                  horizontal: 20 * fss, vertical: 14 * fss),
              decoration: BoxDecoration(
                color: theme.surface,
                borderRadius: BorderRadius.vertical(
                    top: Radius.circular(16 * fss)),
                border:
                Border(bottom: BorderSide(color: theme.divider)),
              ),
              child: Row(children: [
                Icon(Icons.local_pharmacy_rounded,
                    size: 16 * fss, color: theme.accent),
                SizedBox(width: 8 * fss),
                Text(
                  brand.brand_name,
                  style: TextStyle(
...
BoxDecoration _modalDecoration(ThemeDefinition theme, double fss) =>
    BoxDecoration(
      color: theme.surfaceElevated,
      borderRadius: BorderRadius.circular(16 * fss),
      border: Border.all(color: theme.divider),
      boxShadow: [
        BoxShadow(
          color: Colors.black.withOpacity(theme.isDark ? 0.5 : 0.14),
          blurRadius: 40,
...
class _ModalHeader extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      padding: EdgeInsets.fromLTRB(
          20 * fss, 16 * fss, 16 * fss, 14 * fss),
      decoration: BoxDecoration(
        border: Border(bottom: BorderSide(color: theme.divider)),
        borderRadius:
        BorderRadius.vertical(top: Radius.circular(16 * fss)),
      ),
      child: Row(children: [
        Container(
          width: 38 * fss,
          ...
          decoration: BoxDecoration(
            ...
            borderRadius: BorderRadius.circular(10 * fss),
          ),
          child: Icon(icon, size: 18 * fss, color: iconColor),
        ),
        SizedBox(width: 12 * fss),
        Expanded(
          child: Column(
            ...
            children: [
              ...
              Text(
                subtitle,
                style: TextStyle(
                    fontSize: 11 * fss, color: theme.textSecondary),
              ),
            ],
          ),
        ),
          ...
          child: GestureDetector(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                ...
                borderRadius: BorderRadius.circular(7 * fss),
                border: Border.all(color: theme.divider),
              ),
              child: Icon(Icons.close_rounded,
                  size: 14 * fss, color: theme.textSecondary),
            ),
          ),
        ),
      ]),
...
class _CompanyFilter extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      height: 36 * fss,
      padding: EdgeInsets.symmetric(horizontal: 10 * fss),
      decoration: BoxDecoration(
        color: theme.bg,
        borderRadius: BorderRadius.circular(8 * fss),
        border: Border.all(color: theme.divider),
      ),
      child: DropdownButtonHideUnderline(
        child: DropdownButton<int?>(
          value: selectedId,
          hint: Text('All Companies',
              style: TextStyle(
                  fontSize: 12 * fss, color: theme.textSecondary)),
          isExpanded: true,
          dropdownColor: theme.surfaceElevated,
          icon: Icon(Icons.keyboard_arrow_down_rounded,
              size: 16 * fss, color: theme.textSecondary),
          style:
          TextStyle(fontSize: 12 * fss, color: theme.textPrimary),
          items: [
            DropdownMenuItem<int?>(
              value: null,
              child: Text('All Companies',
                  style: TextStyle(
                      fontSize: 12 * fss, color: theme.textPrimary)),
            ),
            ...companyIds.map((id) {
...
class _ModalEmpty extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Padding(
      padding: EdgeInsets.all(40 * fss),
      child: Column(
        mainAxisSize: MainAxisSize.min,
        children: [
          Icon(icon, size: 40 * fss, color: theme.textMuted),
          SizedBox(height: 12 * fss),
          Text(message,
              style: TextStyle(
                  fontSize: 13 * fss, color: theme.textSecondary)),
        ],
      ),
    );
  }
      ...
      child: Column(
        ...
        children: [
          Container(
            ...
            child: Icon(icon, size: 28 * fss, color: theme.textMuted),
          ),
          SizedBox(height: 16 * fss),
          Text(
            title,
            style: TextStyle(
              fontSize: 15 * fss,
          ...
          SizedBox(height: 6 * fss),
          Text(
            subtitle,
            style:
            TextStyle(fontSize: 12 * fss, color: theme.textSecondary),
